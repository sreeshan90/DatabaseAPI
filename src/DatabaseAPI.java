...
public class DatabaseAPI {
	...
	int sameSame() {
		...
		HashMap<HashSet<Integer>, Integer> mapOfInterest = new HashMap<>(); // holds
																			// the
																			// customers
		// having interest in
		// exactly 5 categories or more
		ArrayList<Customer> custList = new ArrayList<>();

		for (Customer c : db.values()) {

			if (c.categories.size() >= 5)
				custList.add(c); // list of cutomers having 5 or more categories
									// of interest
		}

		for (int i = 0; i < custList.size(); i++) {

			if (mapOfInterest.containsKey(custList.get(i).categories)) {
				Integer count = mapOfInterest.get(custList.get(i).categories);
				mapOfInterest.put(custList.get(i).categories, count + 1); // increase
																			// occurance
																			// count

			} else {
				mapOfInterest.put(custList.get(i).categories, 1); // first
																	// occurance
			}
		...
		int customerCount = 0;
		for (Integer s : mapOfInterest.values()) {

			if (s > 1) { // if repeated more than once,
				customerCount += s;
			}
